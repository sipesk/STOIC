...
phylum_tbl <- data$physeq |>
  # Transform into relative abundance
  transform_sample_counts(\(x) x / sum(x)) |>
  psmelt() |>
  group_by(phylum) |>
  summarise(Abundance = mean(Abundance)) |>
  mutate(Phylum_label = if_else(Abundance > 0.025, phylum, "<2.5%")) |>
  column_to_rownames("phylum")
  
phyla <- gsub(" p__", "", phylum_tbl[taxa[rownames(weights), "phylum"], "Phylum_label"])
phyla <- ifelse(phyla=="", "Unknown", phyla)
length(unique(phyla))
...
p1 <- ggnet2(net, 
  ...
  ggtitle("Cryoconite data network plot", subtitle = "Edges show median partial correlation coefficient. We show edges above 0.5 probability of absence (i.e. they were present in more than 50% of 100 repeated rarefactions).")

ggsave("network_plot_cc.pdf", p1, width = 11.69, height = 8.27, units = "in", dpi = "retina")
