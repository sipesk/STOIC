...
2 sample types * 2 count tables * 1 metadata options = 3 phyloseq objects
(ice OR cryoconite)    (site information)
ALLSTOIC is ice and cc MERGED and KATIE25 removed. 
KATIE25 should be looked at against STOIC25 in *"ALLSTOIC_mRNA_siteMD and ALLSTOIC"*
...
#Phyloeq objects
PHY_ice <- phyloseq(ice_rRNA_counts, ice_rRNA_taxa, icesiteMD)

PHY_cc <- phyloseq(cc_rRNA_counts, cc_rRNA_taxa, ccsiteMD)
...
ALLSTOIC <- merge_phyloseq(PHY_ice, PHY_cc)
...
process_phyloseq_objects <- function(...) {
    ...
    for (object_name in object_names) {
        ...
        phy_obj_taxglom <- tax_glom(phy_obj, taxrank = "genus")
...
# ALLSTOIC change taxrank = "genus"
process_phyloseq_objects( ALLSTOIC_no_site6, 
                          PHY_cc_no_site6,
                          PHY_ice_no_site6)
...
rename_phyloseq_objects <- function(object_names) {
    for (original_name in object_names) {
        # Remove '_no_site6_TRUE' from the name and append 'AC_'
...
original_names <- c(
  "PHY_cc_no_site6_TRUE",
  "PHY_ice_no_site6_TRUE",
  "ALLSTOIC_no_site6_TRUE"
)
...
#List of names for the evolution of the files
#Start: 1
#PHY_ice, PHY_cc, ALLSTOIC

#2: Remove "site 6" which is just KATIE25 sample used for sequencing variation
PHY_ice_no_site6, PHY_cc_no_site6, ALLSTOIC_no_site6

3:
"Removes all the '_' identifyers in the taxa names and tax glom on the genus level
PHY_ice_no_site6_TRUE, PHY_cc_no_site6_TRUE, ALLSTOIC_no_site6_TRUE

3: (df version)
PHY_ice_no_site6_df, PHY_cc_no_site6_df, ALLSTOIC_no_site6_df

4 Fix phyloseq object names
AC_ALLSTOIC, AC_PHY_cc, AC_PHY_ice


```


##Phyla bar sites separated, NOT RAREFIED

```{r}


#ALLSTOIC_no_site6_df is the same as the phyloseq object "AC_ALLSTOIC", check to verify

AC_ALLSTOIC_df <-  psmelt(AC_ALLSTOIC)

  
#ALLSTOIC_no_site6_df %>%
AC_ALLSTOIC_df %>%
  ...
  scale_fill_manual(values = get_wants_hue(25)) + #values = taxa_colors) +
  labs(title = "All samples, not rarefied - taxglom on Genera", x = "Day", y = "Relative Abundance", fill = "Phyla") +
...
#Test for rarefraction curve. With this study, we can't toss any samples, so we just rarefy to the smallest libary
...
max_sample <- max(sample_sums(OBJ))
#CC min 931488, max = 3206266
#ICE min 590422, max = 1794355

data <- data.frame(
  Category = c("CC", "CC", "ICE", "ICE"),
  Values = c(931488, 3206266, 590422, 1794355)
)

# Create the dot graph
ggplot(data, aes(x = Category, y = Values)) +
  geom_point(aes(color = Category), size = 3) +
  labs(title = "Min and max library size for CC and ICE", x = "Category", y = "Values") +
  theme_minimal()


#Below is just to look at the library sample sizes
...
###Functionize the major taxa pruning and the rarefying and ploting, with intermediates included. 
```{r}

#AC_PHY_ice == PHY_ice_no_site6_df, 
#AC_PHY_cc == PHY_cc_no_site6_df, 
#AC_ALLSTOIC == ALLSTOIC_no_site6_df

PHYLOSEQOBJECT = AC_PHY_cc

#Taxglom OTU sheet
taxglom_csv <- PHY_cc_no_site6_df  %>%
  select(OTU, Sample, Abundance, phylum, class, order, family, genus) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)

write.csv(taxglom_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/cryoconite_taxglom_cc_df.csv")


#If you want to see the MAJOR MEMBERS before rarefying (those that represent >1% of any of the original samples)
# Step 1: Calculate Total Counts
total_counts <- rowSums(t(otu_table(PHYLOSEQOBJECT)))
print("Total counts per sample:")
print(total_counts)

# Step 2: Calculate Phylum Percentages
phylum_percentages <- (otu_table(PHYLOSEQOBJECT) / total_counts) * 100
print("Phylum percentages:")
print(phylum_percentages)

# Step 3: Identify Major Phyla
major_phyla <- rownames(phylum_percentages)[apply(phylum_percentages, 1, function(x) any(x > 1))]
print("Major phyla (more than 1%):")
print(major_phyla)

# Step 4: Filter for Major Taxa and Display Counts
total_phyla <- length(unique(rownames(otu_table(PHYLOSEQOBJECT))))
print(paste("Starting dataframe of Ice had", total_phyla, "phyla."))
print(paste("There are", length(major_phyla), "major phyla (more than 1%)."))

filtered_physeq <- prune_taxa(major_phyla, PHYLOSEQOBJECT)
print("Filtered physeq:")
print(filtered_physeq)
check <- psmelt(filtered_physeq)

check_csv <- check  %>%
  select(OTU, Sample, Abundance) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)

write.csv(check_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/ice_filteredfor_majortaxa.csv")

# Step 5: Rarefy the Filtered Data, change the variable name to make each rarefied_filteredphyseq_CC and rarefied_filteredphyseq_ICE
rarefied_filteredphyseq_CC <- rarefy_even_depth(
  filtered_physeq, 
  sample.size = min(sample_sums(filtered_physeq)),
  rngseed = FALSE, replace = TRUE, trimOTUs = TRUE, verbose = TRUE
)

#Rarefied after filtering out the <1% members
rarefied_filteredphyseq_ICE

rarefied_filteredphyseq_ICE_df <- psmelt(rarefied_filteredphyseq_ICE)

rarefied_filteredphyseq_ICE_df_csv <- rarefied_filteredphyseq_ICE_df %>%
  select(OTU, Sample, Abundance) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)

write.csv(rarefied_filteredphyseq_ICE_df_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/ICE_filtered_rare_ICE_df.csv")

rarefied_filteredphyseq_CC

rarefied_filteredphyseq_CC_df <- psmelt(rarefied_filteredphyseq_CC)

rarefied_filteredphyseq_CC_df_csv <- rarefied_filteredphyseq_CC_df %>%
  select(OTU, Sample, Abundance) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)

write.csv(rarefied_filteredphyseq_CC_df_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/CC_filtered_rare_CC_df.csv")

###########
#Step 1
#Rarefy without filtering

rarefied_physeq_CC<- rarefy_even_depth(
  AC_PHY_cc , 
  sample.size = min(sample_sums(AC_PHY_cc)),
  rngseed = FALSE, replace = TRUE, trimOTUs = TRUE, verbose = TRUE
)

rarefied_physeq_ICE<- rarefy_even_depth(
  AC_PHY_ice , 
  sample.size = min(sample_sums(AC_PHY_ice)),
  rngseed = FALSE, replace = TRUE, trimOTUs = TRUE, verbose = TRUE
)
...
generate_plot <- function(pseq, plot_title) {
  ...
  p <- ggplot(melted_physeq, aes(x = DAYOFYEAR, y = Abundance, fill = reorder(phylum, Abundance))) + 
    ...
    facet_grid(SITE ~ TYPE) +
    scale_fill_manual(values = get_wants_hue(30)) + # taxa_colors) + 
...
plot_ice <- generate_plot(rarefied_physeq_ICE, "Ice Library Rarefied")
plot_cc <- generate_plot(rarefied_physeq_CC, "Cryoconite Library Rarefied")

# To view the plots, you can use:
print(plot_ice)
print(plot_cc)

CC_rare_df <- psmelt(rarefied_physeq_CC)
ICE_rare_df <- psmelt(rarefied_physeq_ICE)

CC_rare_df_csv <- CC_rare_df %>%
  select(OTU, Sample, Abundance, kingdom, phylum, class, order, family) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)

write.csv(CC_rare_df_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/Cryoconite_rare_df.csv")

ICE_rare_df_csv <- ICE_rare_df %>%
  select(OTU, Sample, Abundance, kingdom, phylum, class, order, family, genus) %>%
  pivot_wider(names_from = Sample, values_from = Abundance)


write.csv(ICE_rare_df_csv, "~/Documents/AU/DeepPurple/Cryobio/STOIC/data/Ice_rare_df.csv")
...
ALLSTOIC_rare <- merge_phyloseq(rarefied_physeq_CC, rarefied_physeq_ICE)
...
# Categorize phyla into Group1 (Prokaryotes, Eukaryotes, Protozoa)
 mutate(
    Group1 = case_when(
      phylum %in% c(" Cyanobacteria", " Proteobacteria", " Bacteroidota", " Myxococcota", 
                    " Armatimonadota", " Acidobacteriota", " Actinobacteriota", " Bdellovibrionota", 
                    " Planctomycetota", " Verrucomicrobiota", " Deinococcota", " Abditibacteriota", 
                    " Gemmatimonadota", " Firmicutes", " Nitrospirota") ~ "Prokaryotes",
      phylum %in% c(" Chlorophyta", " Streptophyta", " Basidiomycota", " Ochrophyta", 
                    " Chytridiomycota", " Rotifera", " Ascomycota", " Nucleariidea", 
                    " Blastocladiomycota", " Opalozoa", " Choanoflagellida (phylum)", 
                    " Cryptophyta", " Pseudofungi") ~ "Eukaryotes",
      phylum %in% c(" Cercozoa", " Ciliophora", " Conosa", " Lobosa", " Dinoflagellata") ~ "Protozoa",
      TRUE ~ "Unassigned"  # Default case for phyla not listed
    )
)

#Need the same color pallete for the taxa
all_taxa <- unique(ALLSTOIC_rare_df$phylum)
set.seed(1000)
number <- length(unique(ALLSTOIC_rare_df$phylum))

colors <- get_wants_hue(number)
taxa_colors <- setNames(colors, all_taxa)

ALLSTOIC_rared_stackbar <- ALLSTOIC_rare_df2  %>%
  mutate(phylum = reorder(phylum, Abundance)) %>%
  ggplot(aes(x= as.factor(DAYOFYEAR), y=Abundance, fill = phylum)) + 
  geom_bar(stat = 'identity', position = "fill" ) +
  scale_y_continuous(label=comma) +
  ...
  labs(title = "All Rarefied Data", x = "Day", y = "Relative Abundance", fill = "Phyla") +
...
phyloseq_obj <- rarefied_physeq_CC
...
#Need to break up the rarefied STOIC data by Eukaryotes, Prokaryotes and Protists, like in Maria's paper. Then show shannon diversity and maybe an NMDS plot at each
...
Euk_barplort <- Euk_ALLSTOIC  %>%
  ...
  facet_grid(SITE~TYPE) +
  scale_fill_manual(values = taxa_colors) +
  labs(title = "Eukaryotic Diversity", x = "Day", y = "Relative Abundance (%)", fill = "Eukaryotic Phyla") +
  scale_x_discrete(breaks=c(209, 213, 217, 219, 222, 222, 225, 230)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
Prok_barplot <- Prok_ALLSTOIC %>%
  ...
  facet_grid(SITE~TYPE) +
  scale_fill_manual(values = taxa_colors) +
  labs(title = "Prokaryotic Diversity", x = "Day", y = "Relative Abundance (%)", fill = "Prokaryotic Phylum") +
  scale_x_discrete(breaks=c(209, 213, 217, 219, 222, 225, 230)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
Prot_barplot <- Prot_ALLSTOIC %>%
  ...
  facet_grid(SITE~TYPE) +
  scale_fill_manual(values = taxa_colors) +
  labs(title = "Protistian Diversity", x = "Day", y = "Realtive Abundance", fill = "Protista Phyla") +
  scale_x_discrete(breaks=c(209, 213, 217, 219, 222, 225, 230)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
Euk_ALLSTOIC_phy_ICE <- subset_samples(Euk_ALLSTOIC_phy , TYPE == "ice")
Euk_ALLSTOIC_phy_CC <- subset_samples(Euk_ALLSTOIC_phy , TYPE == "cryoconite")

#Prokaryotes
Prok_ALLSTOIC_phy <- subset_taxa(ALLSTOIC_rare, phylum %in% c(" Cyanobacteria",  " Acidobacteriota",  " Actinobacteriota",   
                                                              " Bacteroidota",  " Proteobacteria", " Myxococcota",       
                                                              " Armatimonadota", " Planctomycetota", " Bdellovibrionota", 
                                                              " Abditibacteriota", " Deinococcota", " Gemmatimonadota", 
                                                              " Verrucomicrobiota", " Firmicutes", " Nitrospirota" ))

diversity(Prok_ALLSTOIC_phy, index = "all")

Prok_ALLSTOIC_phy_ICE <- subset_samples(Prok_ALLSTOIC_phy , TYPE == "ice")
Prok_ALLSTOIC_phy_CC <- subset_samples(Prok_ALLSTOIC_phy , TYPE == "cryoconite")

#Protists
Protist_ALLSTOIC_phy <- subset_taxa(ALLSTOIC_rare, phylum %in% c(" Ciliophora",  " Conosa",  " Cercozoa", " Lobosa", " Dinoflagellata"))

diversity(Protist_ALLSTOIC_phy, index = "all")

Protist_ALLSTOIC_phy_ICE <- subset_samples(Protist_ALLSTOIC_phy, TYPE == "ice")
Protist_ALLSTOIC_phy_CC <- subset_samples(Protist_ALLSTOIC_phy, TYPE == "cryoconite")
...
AC_PHY_ice@sam_data$SITE <- as.factor(AC_PHY_ice@sam_data$SITE)
...
richness_plot <- ggplot(richness_df$data, aes(x = SITE, y = value)) +
  ...
  theme_bw() +
  labs(title = "Prokaryotic Shannon Diversity by Site", x = "Site", y = "Shannon Diversity Index") +
...
#If shapiro is < 0.05, the data do not follow a normal distribution, do Levene's Test to check varience homogeneity
levenResult <- leveneTest(value ~ SITE, data = richness_data)
levenResult
...
ALLSTOIC_rare_df %>%
  ...
  facet_grid(SITE~TYPE) +
  scale_fill_manual(values = taxa_colors) +
  labs(title = "Rarefied", x = "Day", y = "Relative Abundance", fill = "Phyla") +
  scale_x_discrete(breaks=c(209, 213, 217, 219, 221, 222, 225, 230)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
EUK_compare_df <- subset(ALLSTOIC_no_site6_df, subset = phylum %in% c(" Chytridiomycota", " Streptophyta", " Chlorophyta", "Rhizophydiales"))
...
ALLSTOIC25 = subset_samples(ALLSTOIC, FULLNAME == "0801ICE5")
 

physeq25_df <- psmelt(ALLSTOIC25)
...
phyloseq_objects <- list(
  ...
  rarefied_physeq_CC,
  rarefied_physeq_ICE,
  Protist_ALLSTOIC_phy_CC,
  Protist_ALLSTOIC_phy_ICE,
  Prok_ALLSTOIC_phy_CC,
  Prok_ALLSTOIC_phy_ICE,
  Euk_ALLSTOIC_phy_CC,
  Euk_ALLSTOIC_phy_ICE
)
...
color_vars <- c("DAYOFYEAR")

# Apply function to each phyloseq object and color variable, and print the plots to the viewer
...
rarefied_physeq_ICE_df <- psmelt(rarefied_physeq_ICE)
CC_rare_df <- psmelt(rarefied_physeq_CC)


write.csv((rarefied_physeq_ICE_df), "rarefied_physeq_ICE_df.csv", row.names = FALSE)
write.csv((CC_rare_df), "rarefied_physeq_CC_df.csv", row.names = FALSE)
...
phyloseq_obj <- Euk_ALLSTOIC_phy  # Change this to any of your phyloseq objects
subset_type <- 'ice'              # Choose between 'ice' or 'cryoconite'
phyloseq_name <- "Euk_ALLSTOIC_phy"  # Name of the phyloseq object for plot title and console output
...
#Shannon diversity plots 
plot_shannon_diversity <- function(phyloseq_obj, phyloseq_name) {
  # Ensure that SITE is treated as a factor
  sample_data(phyloseq_obj)$SITE <- as.factor(sample_data(phyloseq_obj)$SITE)

  # Calculate richness (Shannon Index) and create the base plot
  richness_df <- plot_richness(phyloseq_obj, x = "SITE", measures = c("Shannon"))
  ...
  richness_plot <- ggplot(richness_df$data, aes(x = SITE, y = value)) +
    ...
    facet_grid(variable ~ TYPE, scales = "free") +  # Facet by diversity measure and TYPE
    theme_bw() +
    labs(title = paste(phyloseq_name, subset_type, "Shannon Diversity by Site"), x = "Site", y = "Shannon Diversity Index") +
    theme(legend.position = "right",  # Position the legend to the right
          strip.background = element_rect(fill = "white"),
          axis.text.x = element_text(angle = 0, vjust = 0, hjust = 0.5))
...
# Generate NMDS plot
nmds_plot <- plot_phyloseq_nmds(phyloseq_subset, phyloseq_name, "DAYOFYEAR")

# Generate Shannon diversity plot
shannon_plot <- plot_shannon_diversity(phyloseq_subset, phyloseq_name)
...
IceAlgaerRNAsummarize <-  subset(ALLSTOIC_no_site6_df, phylum == " Streptophyta" )
